...
async def orchestrator(host: str, port_str=None, timeout=2) -> None:
    ...
    task_queue = asyncio.Queue(maxsize=MAX_CONSUMERS)
    ...
    tasks = []
    await producer(host, ports, timeout, task_queue, scan_completed)
    ...
    with console.status("[bold green]Scanning..."):
        await scan_completed.wait()  # wait until the producer is done
    ...
    while result_queue.qsize():
        open_ports.append(result_queue.get_nowait())
